...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
    	...
    	String names = binder.bind("spring.datasource.names", String.class).get();
    	if(names == null || "".equals(names)) {
    		// 获取数据源类型
    		Map defauleDataSourceProperties = binder.bind("spring.datasource", Map.class).get();
            String typeStr = (String) defauleDataSourceProperties.get("type");
            ...
            for (int i = 0; i < str.length; i++) {
            	String name = str[i];
        		Map dataSourceProperties = binder.bind("spring.datasource."+name, Map.class).get();
                String typeStr = (String) dataSourceProperties.get("type");
                Class<? extends DataSource> clazz = getDataSourceType(typeStr);
        ...
        logger.info("注册数据源成功，一共注册{}个数据源", targetDataSources.keySet().size());
    }
